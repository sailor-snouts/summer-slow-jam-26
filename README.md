...
The `deploy-itch` job runs automatically after a successful WebGL build (manual dispatch only, since it `needs` that build). All per-project config lives **outside** the workflow, so the same `ci.yml` retargets for every jam without edits:
